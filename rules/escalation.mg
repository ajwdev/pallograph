...
# another identity. The key predicates:
#
...
#
#   effective_can(P, Ns, R, V) — like can/5 but collapses apiGroup (any group
#       matches) and includes permissions reachable via escalation.
#
...
#
# Wildcard handling: can/5 stores "*" literals for apiGroup, resource, and verb.
...
perm_sa_token_create(P, Ns) :- can(P, Ns, "",  "serviceaccounts/token", "create").
perm_sa_token_create(P, Ns) :- can(P, Ns, "",  "serviceaccounts/token", "*").
perm_sa_token_create(P, Ns) :- can(P, Ns, "",  "*",                     "create").
perm_sa_token_create(P, Ns) :- can(P, Ns, "",  "*",                     "*").
perm_sa_token_create(P, Ns) :- can(P, Ns, "*", "serviceaccounts/token", "create").
perm_sa_token_create(P, Ns) :- can(P, Ns, "*", "serviceaccounts/token", "*").
perm_sa_token_create(P, Ns) :- can(P, Ns, "*", "*",                     "create").
perm_sa_token_create(P, Ns) :- can(P, Ns, "*", "*",                     "*").

perm_pods_exec_create(P, Ns) :- can(P, Ns, "",  "pods/exec", "create").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "",  "pods/exec", "*").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "",  "*",         "create").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "",  "*",         "*").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "*", "pods/exec", "create").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "*", "pods/exec", "*").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "*", "*",         "create").
perm_pods_exec_create(P, Ns) :- can(P, Ns, "*", "*",         "*").

perm_pods_attach_create(P, Ns) :- can(P, Ns, "",  "pods/attach", "create").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "",  "pods/attach", "*").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "",  "*",           "create").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "",  "*",           "*").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "*", "pods/attach", "create").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "*", "pods/attach", "*").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "*", "*",           "create").
perm_pods_attach_create(P, Ns) :- can(P, Ns, "*", "*",           "*").

perm_pods_create(P, Ns) :- can(P, Ns, "",  "pods", "create").
perm_pods_create(P, Ns) :- can(P, Ns, "",  "pods", "*").
perm_pods_create(P, Ns) :- can(P, Ns, "",  "*",    "create").
perm_pods_create(P, Ns) :- can(P, Ns, "",  "*",    "*").
perm_pods_create(P, Ns) :- can(P, Ns, "*", "pods", "create").
perm_pods_create(P, Ns) :- can(P, Ns, "*", "pods", "*").
perm_pods_create(P, Ns) :- can(P, Ns, "*", "*",    "create").
perm_pods_create(P, Ns) :- can(P, Ns, "*", "*",    "*").

perm_sa_impersonate(P, Ns) :- can(P, Ns, "",  "serviceaccounts", "impersonate").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "",  "serviceaccounts", "*").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "",  "*",               "impersonate").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "",  "*",               "*").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "*", "serviceaccounts", "impersonate").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "*", "serviceaccounts", "*").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "*", "*",               "impersonate").
perm_sa_impersonate(P, Ns) :- can(P, Ns, "*", "*",               "*").

perm_user_impersonate(P, Ns) :- can(P, Ns, "",  "users", "impersonate").
perm_user_impersonate(P, Ns) :- can(P, Ns, "",  "users", "*").
perm_user_impersonate(P, Ns) :- can(P, Ns, "",  "*",     "impersonate").
perm_user_impersonate(P, Ns) :- can(P, Ns, "",  "*",     "*").
perm_user_impersonate(P, Ns) :- can(P, Ns, "*", "users", "impersonate").
perm_user_impersonate(P, Ns) :- can(P, Ns, "*", "users", "*").
perm_user_impersonate(P, Ns) :- can(P, Ns, "*", "*",     "impersonate").
perm_user_impersonate(P, Ns) :- can(P, Ns, "*", "*",     "*").

perm_group_impersonate(P, Ns) :- can(P, Ns, "",  "groups", "impersonate").
perm_group_impersonate(P, Ns) :- can(P, Ns, "",  "groups", "*").
perm_group_impersonate(P, Ns) :- can(P, Ns, "",  "*",      "impersonate").
perm_group_impersonate(P, Ns) :- can(P, Ns, "",  "*",      "*").
perm_group_impersonate(P, Ns) :- can(P, Ns, "*", "groups", "impersonate").
perm_group_impersonate(P, Ns) :- can(P, Ns, "*", "groups", "*").
perm_group_impersonate(P, Ns) :- can(P, Ns, "*", "*",      "impersonate").
perm_group_impersonate(P, Ns) :- can(P, Ns, "*", "*",      "*").
...
# ---- effective_can ----
#
# Collapses apiGroup out of can/5 — callers care about (Ns, R, V), not which
# group the grant came from. Wildcard resource/verb values in can/5 are passed
# through as-is; callers should be aware that effective_can(P, Ns, "*", V) means
# the principal has a wildcard-resource grant, not a grant for the literal resource "*".

effective_can(P, Ns, R, V) :- can(P, Ns, _, R, V).

effective_can(P, Ns, R, V) :-
    controls_identity(P, Target),
    can(Target, Ns, _, R, V).
